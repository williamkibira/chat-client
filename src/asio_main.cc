...
auto main(int argc, char **argv) -> int
{
  ...
  client->registerInstructionReceiver([&instructor, &console](ResponseType responseType, const std::vector<uint8_t>& buffer) {
    console->info("GOT INSTRUCTION");
    instructor.onReceived(responseType, buffer);
  });
